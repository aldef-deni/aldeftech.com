...
{{-- Solutions Grid (Aldef Dark & Navy Signature Background) --}}
<section class="section-padding bg-gradient-to-b from-[#090E1A] via-[#0C1427] to-[#080D18] relative text-slate-300 border-b border-slate-800/80">
    ...
    <div class="max-w-7xl mx-auto px-5 sm:px-8 lg:px-10 relative z-10">
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            ...
            <div class="bg-[#0F172A]/90 backdrop-blur-xl border border-white/10 rounded-3xl p-8 flex flex-col justify-between group transition-all duration-300 hover:border-blue-500/40 hover:shadow-[0_24px_50px_-10px_rgba(37,99,235,0.25)] hover:-translate-y-1.5 reveal reveal-delay-{{ min($loop->iteration, 3) }}">
                <div>
                    {{-- Icon Container & Badge --}}
                    <div class="flex items-center justify-between mb-6">
                        <div class="w-14 h-14 rounded-2xl bg-gradient-to-br from-blue-500 to-indigo-600 text-white flex items-center justify-center text-2xl shadow-lg shadow-blue-500/25 group-hover:scale-110 group-hover:rotate-3 transition-transform duration-300">
                            {{ $solution->icon ?? '📦' }}
                        </div>
                        <span class="text-[0.6875rem] font-mono font-bold px-2.5 py-1 rounded-full text-blue-300 bg-blue-950/80 border border-blue-800/60">
                            Enterprise Ready
                        </span>
                    </div>

                    <h2 class="text-xl font-display font-bold text-white mb-3 group-hover:text-blue-300 transition-colors">
                        {{ $solution->title }}
                    </h2>
                    ...
                    <div class="space-y-2 pt-5 border-t border-white/10 mb-6">
                        @foreach($solution->features as $feature)
                        <div class="flex items-start gap-2.5 text-xs text-slate-200 bg-white/[0.04] border border-white/[0.06] px-3.5 py-2 rounded-xl">
                            <svg class="w-4 h-4 text-emerald-400 shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M5 13l4 4L19 7"/></svg>
                ...
                <div class="pt-5 border-t border-white/10 flex items-center justify-between">
                    <a href="{{ \App\Services\WhatsAppService::getUrl() }}?text=Halo%20Aldef%20Tech,%20saya%20tertarik%20dengan%20solusi%20{{ urlencode($solution->title) }}"
                       ...
                       class="text-xs font-bold uppercase tracking-wider text-blue-400 hover:text-blue-300 inline-flex items-center gap-1.5 group/link">
