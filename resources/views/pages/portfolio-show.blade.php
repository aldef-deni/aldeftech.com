...
@php
    ...
    $narrative = array_filter([
        ...
        __('pages.portfolio.detail.approach')  => $portfolio->approach,
        __('pages.portfolio.detail.solution')  => $portfolio->solution,
        __('pages.portfolio.detail.results')   => $portfolio->results,
    ]);
@endphp
...
<x-page-hero
    :eyebrow="$portfolio->category->name ?? __('pages.portfolio.detail.fallback_case')"
...
<section class="surface-ivory-deep border-b border-line">
    <div class="shell">
        <dl class="grid grid-cols-2 lg:grid-cols-4 gap-y-8 gap-x-6 py-9 lg:py-11" data-reveal-group="70">
            @if($portfolio->client)
            <div class="reveal">
                <dt class="eyebrow">{{ __('pages.portfolio.detail.client') }}</dt>
            ...
            @if($portfolio->year)
            <div class="reveal">
                <dt class="eyebrow">{{ __('pages.portfolio.detail.year') }}</dt>
            ...
            <div class="reveal">
                <dt class="eyebrow">{{ __('pages.portfolio.detail.category') }}</dt>
            ...
            <div class="reveal">
                <dt class="eyebrow">{{ __('pages.portfolio.detail.link') }}</dt>
                <dd class="mt-3">
                    <a href="{{ $portfolio->project_url }}" target="_blank" rel="noopener" class="link-arrow">
                        <span>{{ __('pages.portfolio.detail.visit') }}</span>
            ...
            <aside class="lg:col-span-4">
                <div class="lg:sticky lg:top-28 space-y-5">
                    <div class="card-lux card-lux-featured reveal p-7">
                        <p class="eyebrow">{{ __('pages.portfolio.detail.similar_eyebrow') }}</p>
                        <h2 class="mt-4 text-xl leading-snug">
                            {{ __('pages.portfolio.detail.similar_title') }} <span class="accent-serif accent-gold">{{ __('pages.portfolio.detail.similar_accent') }}</span> {{ __('pages.portfolio.detail.similar_after') }}
                        </h2>
    ...
    <div class="shell">
        ...
        <div class="mt-8 cards-swipe md:grid md:grid-cols-3 gap-5 lg:gap-6" data-reveal-group="80">
            ...
            <a href="{{ route('portfolio.show', $related->slug) }}" class="card-lux reveal group overflow-hidden">
                ...
                <div class="p-6">
                    <p class="text-[0.6875rem] uppercase tracking-[0.14em] text-gold-700">{{ $related->category->name ?? __('site.common.project') }}</p>
