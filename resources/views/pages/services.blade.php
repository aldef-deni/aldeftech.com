...
{{-- Services Grid (Aldef Dark & Navy Signature Background) --}}
<section class="section-padding bg-gradient-to-b from-[#090E1A] via-[#0C1427] to-[#080D18] relative text-slate-300 border-b border-slate-800/80">
    ...
    <div class="max-w-7xl mx-auto px-5 sm:px-8 lg:px-10 relative z-10">
        <div class="space-y-10">
            ...
            <div class="bg-[#0F172A]/90 backdrop-blur-xl border border-white/10 rounded-3xl p-8 lg:p-12 shadow-[0_16px_40px_-10px_rgba(0,0,0,0.5)] hover:border-blue-500/40 hover:shadow-[0_24px_50px_-10px_rgba(37,99,235,0.25)] hover:-translate-y-1 transition-all duration-300 group reveal reveal-delay-{{ min($loop->iteration, 3) }}" id="{{ $service->slug }}">
                <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-10 items-start">
                    
                    {{-- Service Header & Icon --}}
                    <div class="lg:col-span-4">
                        <div class="w-16 h-16 rounded-2xl bg-gradient-to-br from-blue-500 to-indigo-600 text-white flex items-center justify-center text-3xl mb-6 shadow-lg shadow-blue-500/25 group-hover:scale-105 transition-transform duration-300">
                            {{ $service->icon ?? '⚡' }}
                        </div>
                        <h2 class="text-2xl lg:text-3xl font-display font-extrabold text-white mb-3 tracking-tight group-hover:text-blue-300 transition-colors">
                            {{ $service->title }}
                        </h2>
                    ...
                    {{-- Service Details & Features: Soft Warm Yellow-Orange Background --}}
                    <div class="lg:col-span-8 bg-gradient-to-br from-[#FFFBEB] via-[#FEF3C7] to-[#FFEDD5] border border-amber-300/90 rounded-2xl p-6 lg:p-8 shadow-md">
                        ...
                        <div>
                            <div class="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-lg bg-amber-200/90 border border-amber-300 text-amber-950 text-xs font-mono font-bold uppercase tracking-wider mb-4">
                                <span>⚡</span>
                                <span>Key Capabilities & Features</span>
                            </div>
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                @foreach($service->features as $feature)
                                <div class="flex items-center gap-2.5 text-xs sm:text-sm text-slate-900 bg-white/95 border border-amber-200/90 px-3.5 py-2.5 rounded-xl shadow-2xs">
                                    <svg class="w-4 h-4 text-amber-600 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M5 13l4 4L19 7"/></svg>
