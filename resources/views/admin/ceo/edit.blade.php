...
{{-- ══════════════════════════════════════════════════════════════════════
     PROFIL KOMISARIS
     Same fields and uploader as the CEO profile above; shown on the homepage
     right below the CEO with the mirrored layout.
     ══════════════════════════════════════════════════════════════════════ --}}
<form method="POST" action="{{ route('admin.ceo.commissioner.update') }}" id="komisaris" class="mt-5">
    @csrf
    @method('PUT')
    ...
    <x-admin.page-head
        ...
        subtitle="Tampil di beranda tepat di bawah profil CEO">
        <button type="submit" class="btn btn-primary">
            <i class="icon-base ti tabler-device-floppy me-2"></i>Simpan Perubahan
        </button>
    </x-admin.page-head>
    ...
    <div class="row g-4">
        <div class="col-12 col-lg-8">
            ...
            <div class="card mb-4">
                ...
                <div class="card-body">
                    <div class="row">
                        <x-admin.form.input
                            col="12 col-md-6" label="Nama" name="name" :value="$commissioner->name" required />
                        <x-admin.form.input
                            col="12 col-md-6" label="Jabatan" name="position" :value="$commissioner->position" required
                            placeholder="mis. Komisaris" />
                    </div>

                    <x-admin.form.textarea
                        label="Deskripsi / Kutipan" name="short_bio" :value="$commissioner->short_bio ?? ''" :rows="4"
                        placeholder="Kalimat yang dikutip besar di beranda"
                        help="Dipakai sebagai kutipan di bagian Kepemimpinan. Maksimal 1000 karakter." />

                    <x-admin.form.textarea
                        label="Deskripsi Lengkap" name="full_bio" :value="$commissioner->full_bio ?? ''" :rows="8"
                        placeholder="Latar belakang, pengalaman, dan fokus keahlian" />
                </div>
            </div>
            ...
            <x-admin.form.translation
                ...
                :fields="[
                    'position'   => ['label' => 'Jabatan (English)', 'type' => 'text'],
                    'short_bio'  => ['label' => 'Deskripsi / Kutipan (English)', 'type' => 'textarea', 'rows' => 4],
                    'full_bio'   => ['label' => 'Deskripsi Lengkap (English)', 'type' => 'textarea', 'rows' => 8],
                    'skills'     => ['label' => 'Bidang Keahlian (English)', 'type' => 'list'],
                    'experience' => ['label' => 'Pengalaman (English)', 'type' => 'list'],
                ]" />
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <div class="row">
                        <x-admin.form.input
                            col="12 col-md-6" label="Email" name="email" type="email" :value="$commissioner->email ?? ''" />
                        <x-admin.form.input
                            col="12 col-md-6" label="LinkedIn" name="linkedin" type="url" :value="$commissioner->linkedin ?? ''"
                            placeholder="https://linkedin.com/in/..." />
                        <x-admin.form.input
                            col="12 col-md-6" label="GitHub" name="github" type="url" :value="$commissioner->github ?? ''"
                            placeholder="https://github.com/..." />
                        <x-admin.form.input
                            col="12 col-md-6" label="Instagram" name="instagram" type="url" :value="$commissioner->instagram ?? ''"
                            placeholder="https://instagram.com/..." />
                    </div>
                </div>
            </div>
        ...
        <div class="col-12 col-lg-4">
            ...
            <div class="card mb-4">
                ...
                <div class="card-body">
                    <x-admin.form.image
                        label="Foto" name="profile_photo" :value="$commissioner->profile_photo ?? ''"
                        ratio="4 / 5"
                        help="Potret rasio 4:5 paling pas. Tanpa foto, profil tetap tampil tanpa gambar." />

                    <x-admin.form.switch
                        label="Tampilkan di situs" name="is_active" :checked="$commissioner->is_active ?? true" />
                </div>
            </div>
            ...
            <div class="card mb-4">
                ...
                <div class="card-body">
                    <x-admin.form.list
                        label="Bidang Keahlian" name="skills"
                        :items="$commissioner->skills ?? []"
                        placeholder="mis. Tata Kelola Perusahaan"
                        add-label="Tambah keahlian" />
                </div>
            </div>
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <x-admin.form.list
                        label="Riwayat" name="experience"
                        :items="$commissioner->experience ?? []"
                        placeholder="mis. 10+ tahun tata kelola perusahaan"
                        add-label="Tambah pengalaman" />
                </div>
            </div>
        </div>
