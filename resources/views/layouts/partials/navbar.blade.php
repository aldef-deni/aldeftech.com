...
<nav id="navbar" class="fixed top-0 left-0 right-0 z-50 navbar-blur">
        ...
        <div class="flex items-center justify-between h-18 lg:h-20">
            ...
            <a href="{{ route('home') }}" class="flex items-center gap-2 shrink-0 group">
                <img src="{{ asset('images/logo.png') }}" alt="{{ config('app.name') }}"
                     class="h-9 lg:h-10 w-auto transition-all duration-300 group-hover:brightness-110 group-hover:drop-shadow-[0_0_12px_rgba(168,85,247,0.3)]">
            </a>
            ...
            <div class="hidden lg:flex items-center gap-1">
                ...
                <a href="{{ route($link['route']) }}"
                   class="text-[0.8125rem] font-medium px-3.5 py-2 rounded-lg transition-all duration-200 {{ request()->routeIs($link['route'].'*') ? 'text-text-primary bg-white/[0.04]' : 'text-text-muted hover:text-text-primary hover:bg-white/[0.03]' }}">
                    {{ $link['label'] }}
                </a>
                @endforeach
            </div>
            ...
            <div class="flex items-center gap-3">
                ...
                {{-- Mobile Hamburger --}}
                <button @click="mobileMenuOpen = !mobileMenuOpen" class="lg:hidden text-text-muted hover:text-text-primary p-2 -mr-2 rounded-lg transition-colors hover:bg-white/[0.03]">
    ...
    <div x-show="mobileMenuOpen" x-cloak
         ...
         class="lg:hidden bg-brand-surface/95 backdrop-blur-2xl border-t border-brand-border">
        <div class="px-5 py-5 space-y-1">
            @foreach($navLinks as $link)
            <a href="{{ route($link['route']) }}"
               class="block px-4 py-3 rounded-xl text-sm font-medium transition-all {{ request()->routeIs($link['route'].'*') ? 'text-text-primary bg-accent/5 border border-accent/10' : 'text-text-muted hover:text-text-primary hover:bg-white/[0.03]' }}">
                {{ $link['label'] }}
            </a>
            @endforeach
