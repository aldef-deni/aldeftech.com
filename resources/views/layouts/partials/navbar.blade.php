...
<nav id="navbar" class="fixed top-0 left-0 right-0 z-50 navbar-blur">
    <div class="max-w-7xl mx-auto px-5 sm:px-8 lg:px-10">
        <div class="flex items-center justify-between h-18 lg:h-20">
            ...
            {{-- Desktop Navigation --}}
            <div class="hidden lg:flex items-center gap-1 bg-slate-50/80 border border-slate-200/80 p-1.5 rounded-full shadow-xs">
                @php
                $navLinks = [
                ...
                <a href="{{ route($link['route']) }}"
                   class="text-[0.8125rem] font-medium px-4 py-1.5 rounded-full transition-all duration-200 {{ request()->routeIs($link['route'].'*') ? 'text-blue-600 bg-white shadow-xs font-semibold' : 'text-slate-600 hover:text-slate-900 hover:bg-white/60' }}">
                    {{ $link['label'] }}
                </a>
                @endforeach
            </div>
    ...
    <div x-show="mobileMenuOpen" x-cloak
        ...
        <div class="px-5 py-5 space-y-1.5">
            @foreach($navLinks as $link)
            <a href="{{ route($link['route']) }}"
               class="block px-4 py-2.5 rounded-xl text-sm font-medium transition-all {{ request()->routeIs($link['route'].'*') ? 'text-blue-600 bg-blue-50/80 font-semibold' : 'text-slate-600 hover:text-slate-900 hover:bg-slate-50' }}">
                {{ $link['label'] }}
            </a>
            @endforeach
